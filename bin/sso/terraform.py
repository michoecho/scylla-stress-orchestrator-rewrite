...
def apply(terraform_plan, config, deployment_name):
    ...
    #with open(f'{deployment_name}/environment.yml', 'w') as f:
    #    documents = yaml.dump(environment, f)
...
def destroy(terraform_plan, deployment_name):
    if not path.isdir(terraform_plan):
        print(f"Could not find directory [{terraform_plan}]")
        exit(1)
